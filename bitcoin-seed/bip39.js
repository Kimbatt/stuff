...
/**
 * 
 * @param {String | Number[] | Uint8Array} password 
 * @param {String | Number[] | Uint8Array} salt 
 ...
 * @param {String} salt 
 */
async function PBKDF2_Worker(password, salt)
{
    return new Promise(resolve => EnqueueWorkerTask({ _type: "PBKDF2", _data: [password, salt] }, resolve));
}
...
function GenerateSeedPhrase(wordCount)
{
    ...
    bits += ((checksumByte >> (8 - checksumLength)) & ((1 << checksumLength) - 1)).toString(2).padStart(checksumLength, "0");

    const words = [];
    for (let i = 0; i < bits.length; i += 11)
    {
        const index = Number.parseInt(bits.substr(i, 11), 2);
...
function VerifyMnemonic(mnemonic)
{
    ...
    const words = mnemonic.match(/\w+/g);
    ...
    {
        seedBytes.push(Number.parseInt(seedBits.substr(i, 8), 2));
    }

    const actualChecksumByte = SHA256(seedBytes)[0];
    const actualChecksumBits = actualChecksumByte.toString(2).padStart(8, "0").substr(0, checksumLength);
 ...
 * @param {String} password 
 */
async function GetXprvFromMnemonic(mnemonic, password = "", purpose = "44")
{
    const mnemonicBytes = Utf8StringToBytes(mnemonic.normalize("NFKD"));
    const passwordBytes = Utf8StringToBytes(("mnemonic" + password).normalize("NFKD"));
    
    pbkdf2Loading = true;
    ++activeWorkers;
    const seed = await PBKDF2_Worker(mnemonicBytes, passwordBytes, 2048, 512/32);
    --activeWorkers;
...
const DOM = {
    wordCountComboBox: document.getElementById("seed-wordcount"),
    seedTextarea: document.getElementById("seed-textarea"),
    seedPassphraseTextArea: document.getElementById("seed-passphrase"),
    seedBIP32RootKeyTextarea: document.getElementById("seed-bip32-rootkey"),
    seedErrorText: document.getElementById("seed-error-text"),
    addressesErrorText: document.getElementById("addresses-error-text"),
    loadingSpinner: document.getElementById("loading-spinner"),
    loadingSpinnerAddresses: document.getElementById("loading-spinner-addresses"),
    derivationPathPresetComboBox: document.getElementById("derivation-path-preset"),
    generateChangeAddressesCheckbox: document.getElementById("generateChangeAddressesCheckbox"),
    generateChangeAddressesLabel: document.getElementById("generateChangeAddressesLabel"),
    derivationPathTextbox: document.getElementById("derivation-path"),
    generateHardenedAddressesCheckbox: document.getElementById("generateHardenedAddressesCheckbox"),
    derivedAddressesDiv: document.getElementById("derived-addresses"),
    derivedAddressCount: document.getElementById("derived-address-count"),
    derivedAddressOffset: document.getElementById("derived-address-offset"),
    calculateDerivedAddressesButton: document.getElementById("calculateDerivedAddressesButton"),
    extendedPublicKeyTextarea: document.getElementById("extended-public-key"),
    extendedPrivateKeyTextarea: document.getElementById("extended-private-key"),
};

function ShowErrorText(errorTextElement, error)
{
    errorTextElement.innerText = error;
    errorTextElement.style.visibility = "";
}
...
function GenerateNewSeedButtonClick()
{
    const newSeed = GenerateSeedPhrase(DOM.wordCountComboBox.value);
    DOM.seedTextarea.value = newSeed;
    SeedChanged();
}
...
const changedWaitTime = 150;
let waitingCount = 0;
async function ShouldSkip()
{
    ++waitingCount;
    await WaitFor(changedWaitTime);
    --waitingCount;
    return waitingCount !== 0;
}

async function SeedChanged()
{
    const mnemonic = DOM.seedTextarea.value;
    const verifyResult = VerifyMnemonic(mnemonic);
    if (typeof verifyResult === "string")
    {
        ShowErrorText(DOM.seedErrorText, "Invalid seed value: " + verifyResult);
        DOM.calculateDerivedAddressesButton.disabled = true;
        return;
    }

    const wordCount = mnemonic.match(/\w+/g).length;
    DOM.wordCountComboBox.value = wordCount;

    HideErrorText(DOM.seedErrorText);
    ShowLoadingSpinner(DOM.loadingSpinner);

    if (await ShouldSkip())
        return;

    const passphrase = DOM.seedPassphraseTextArea.value;
    const bip32RootKey = await GetXprvFromMnemonic(mnemonic, passphrase);

    if (activeWorkers !== 0)
        return;

    DOM.seedBIP32RootKeyTextarea.value = bip32RootKey;
    DOM.calculateDerivedAddressesButton.disabled = false;

    HideLoadingSpinner(DOM.loadingSpinner);
}

async function SeedPassphraseChanged()
{
    ShowLoadingSpinner(DOM.loadingSpinner);
...
function SeedBIP32RootKeyWarnIfNotEmpty(ev)
{
    if (ev.key.length !== 1 && ev.key !== "Backspace" && ev.key !== "Delete")
        return;

    if (DOM.seedTextarea.value !== "" || DOM.seedPassphraseTextArea.value !== "")
    {
        if (!confirm("Changing the BIP32 root key will clear the seed and the passphrase field. Continue?"))
        {
            ev.preventDefault();
            ev.stopPropagation();
        }
...
function SeedBIP32RootKeyChanged()
{
    DOM.seedTextarea.value = "";
    DOM.seedPassphraseTextArea.value = "";

    let decodedKey;
    const rootKey = DOM.seedBIP32RootKeyTextarea.value;
    try
    {
        decodedKey = base58decode(rootKey);
    }
    catch (e)
    {
        ShowErrorText(DOM.seedErrorText, "Invalid BIP32 root key: " + e.message);
        DOM.calculateDerivedAddressesButton.disabled = true;
        return;
    }
    ...
    {
        ShowErrorText(DOM.seedErrorText, "Invalid BIP32 root key: decoded key length must be 78");
        DOM.calculateDerivedAddressesButton.disabled = true;
        return;
    }
    ...
    if (rootKey[0] === "y")
    {
        DOM.derivationPathPresetComboBox.value = "49";
        DerivationPathChanged();
    }
    else if (rootKey[0] === "z")
    {
        DOM.derivationPathPresetComboBox.value = "84";
        DerivationPathChanged();
    }
}

function DerivationPathChanged()
{
    const newValue = DOM.derivationPathPresetComboBox.value;
    switch (newValue)
    {
        case "32":
            DOM.derivationPathTextbox.readOnly = false;
            DOM.derivationPathTextbox.value = "";
            DOM.generateChangeAddressesLabel.style.display = "none";
            break;
        case "44":
            DOM.derivationPathTextbox.readOnly = true;
            DOM.derivationPathTextbox.value = "m/44'/0'/0'";
            DOM.generateChangeAddressesLabel.style.display = "";
            break;
        case "49":
            DOM.derivationPathTextbox.readOnly = true;
            DOM.derivationPathTextbox.value = "m/49'/0'/0'";
            DOM.generateChangeAddressesLabel.style.display = "";
            break;
        case "84":
            DOM.derivationPathTextbox.readOnly = true;
            DOM.derivationPathTextbox.value = "m/84'/0'/0'";
            DOM.generateChangeAddressesLabel.style.display = "";
            break;
    }
}
...
let isCalculatingDerivedAddresses = false;
async function CalculateDerivedAddresses()
{
    if (isCalculatingDerivedAddresses)
        return;

    count = Number(DOM.derivedAddressCount.value);
    if (isNaN(count))
    {
        count = 20;
        DOM.derivedAddressCount.value = count;
    }

    if (count > 100)
    {
        if (!confirm("Generating " + count +" addresses can take a long time. Continue?"))
            return;
    }

    const rootKey = DOM.seedBIP32RootKeyTextarea.value;
    let path = DOM.derivationPathTextbox.value;
    const generateHardenedAddresses = DOM.generateHardenedAddressesCheckbox.checked;
    let derivedKeyPurpose = DOM.derivationPathPresetComboBox.value;

    let startIndex = Number(DOM.derivedAddressOffset.value);
    if (isNaN(startIndex))
    {
        startIndex = 0;
        DOM.derivedAddressOffset.value = startIndex;
    }
    const endIndex = startIndex + count;

    if (endIndex > 0x80000000)
    {
        ShowErrorText(DOM.addressesErrorText, "Error: Start index + Count must be 2147483648 at most");
        return;
    }

    if (startIndex < 0)
    {
        ShowErrorText(DOM.addressesErrorText, "Error: Start index must be non-negative");
        return;
    }
    
    if (count < 1)
    {
        ShowErrorText(DOM.addressesErrorText, "Error: Count must be at least 1");
        return;
    }
    ...
    {
        const generateChangeAddresses = DOM.generateChangeAddressesCheckbox.checked;
    ...
    {
        ShowErrorText(DOM.addressesErrorText, "Error: Hardened addresses can only be derived from extended private keys");
        return;
    }

    HideErrorText(DOM.addressesErrorText);
    ShowLoadingSpinner(DOM.loadingSpinnerAddresses);
    isCalculatingDerivedAddresses = true;
    DOM.calculateDerivedAddressesButton.disabled = true;

    let derivedExtendedPrivateKey;
    let derivedExtendedPublicKey;
    try
    {
        derivedExtendedPublicKey = await DeriveKey(rootKey, path, false, derivedKeyPurpose);
        if (isPrivate)
            derivedExtendedPrivateKey = await DeriveKey(rootKey, path, isPrivate, derivedKeyPurpose);
    }
    catch (e)
    {
        ShowErrorText(DOM.addressesErrorText, e);
        HideLoadingSpinner(DOM.loadingSpinnerAddresses);
        isCalculatingDerivedAddresses = false;
        DOM.calculateDerivedAddressesButton.disabled = false;
        return;
    }

    DOM.extendedPublicKeyTextarea.value = derivedExtendedPublicKey;
    DOM.extendedPrivateKeyTextarea.value = isPrivate ? derivedExtendedPrivateKey : "???";

    const createDiv = function(addressPath, address, privkey)
    {
        const div = document.createElement("div");
        div.className = "derived-address-container";

        const pathDiv = document.createElement("div");
        pathDiv.style = "display: inline-block; width: 320px;";
        pathDiv.innerText = addressPath;

        const addressDiv = document.createElement("div");
        addressDiv.style = "display: inline-block; width: 600px;";
        addressDiv.innerText = address;

        const privkeyDiv = document.createElement("div");
        privkeyDiv.style = "display: inline-block;";
        privkeyDiv.innerText = privkey;

        div.appendChild(pathDiv);
        div.appendChild(addressDiv);
        div.appendChild(privkeyDiv);

        DOM.derivedAddressesDiv.appendChild(div);
    };

    while (DOM.derivedAddressesDiv.lastChild)
        DOM.derivedAddressesDiv.removeChild(DOM.derivedAddressesDiv.lastChild);

    const promises = new Array(count);
    const divData = new Array(count);
    for (let i = startIndex; i < endIndex; ++i)
    {
        const index = i - startIndex;
        const _i = i;

        promises[index] = new Promise(async resolve =>
        {
            const privkey = isPrivate ?
                UnextendKey(await DeriveKey(derivedExtendedPrivateKey, "m/" + _i + (generateHardenedAddresses ? "'" : ""), true, derivedKeyPurpose))
                : "???";
            const address = UnextendKey(await DeriveKey(
                isPrivate ? derivedExtendedPrivateKey : derivedExtendedPublicKey, "m/" + _i + (generateHardenedAddresses ? "'" : ""), false, derivedKeyPurpose));
            const addressPath = path + (path.endsWith("/") ? "" : "/") + _i + (generateHardenedAddresses ? "'" : "");

            divData[index] = [addressPath, address, privkey];
            resolve();
        });
    }
    ...
    divData.forEach(data => createDiv(data[0], data[1], data[2]));
